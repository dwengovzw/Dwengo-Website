...
function loadLearningPaths(filter = "", lang = "") {
    ...
    xhttp.open("GET", "http://83.217.67.53/backend/api/learningPath/search?all=" + filter + "&language=" + lang, true);
    xhttp.send();
}
...
function loadObjectContent(hruid, language, version) {
    ...
    xhttp.open("GET", `http://83.217.67.53/backend/api/learningObject/getRaw?hruid=${hruid}&version=${version}&language=${language}`, true);
    xhttp.send();
}
...
function visualizeLearningPath(path) {
    ...
    while (counter < path.nodes.length) {
        ...
        xhttp.open("GET", `http://83.217.67.53/backend/api/learningObject/getMetadata?hruid=${node.learningobject_hruid}&version=${node.version}&language=${node.language}`, true);
        xhttp.send();
...
function loadLearningPath() {
    ...
    xhttp.open("GET", "http://83.217.67.53/backend/api/learningPath/" + id, true);
    xhttp.send();
...
function loadFilters() {
    ...
    xhttp.open("GET", "http://83.217.67.53/backend/api/learningPath/languages", true);
    xhttp.send();
